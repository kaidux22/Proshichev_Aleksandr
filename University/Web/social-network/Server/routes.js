...
io.on("connection", (socket) => {
    ...
    socket.on("chat", (data) => {
        io.emit(data.chat_id, data.msgs)
    })
})
...
router.get("/dltPhoto:id", (req, res) => {
    fs.unlink('../Data/' + req.params.id + '/avatar.jpg', err => {
        res.end('true')
    })
})

router.get("/avatar:id.jpg", (req, res) => {
    let cur_path = path.resolve(__dirname, '../Data', req.params.id, "avatar.jpg")

    if(!fs.existsSync(cur_path)){
        cur_path = path.join(__dirname, '/views', '/src',  "avatar.svg")
    }
...
router.post("/addUser", jsonParser, (req, res) => {
    ...
    //fs.open("../Data/" + req.body.data.id.toString() + "/news.json", "w", (err) =>{if(err)throw err})
...
const multer = require('multer'),
    storage = multer.diskStorage({
        destination: function(req, file, callback) {
            callback(null, __dirname + '/../Data/' + req.params.id);
        },
...
router.post("/updPhoto:id", upload.any(), (req, res) => {
    res.end('true')
})
