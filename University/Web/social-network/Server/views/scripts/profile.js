...
$('.menu button').on("click", function (event) {
  if (event.currentTarget.className == "role") {
    if (confirm("Вы точно уверены, что хотите сделать пользователя администратором?")) $.post("/changeRole", {}).done(function () {
      setTimeout(function () {
        location.reload();
      }, 500);
    });
  } else if (event.currentTarget.className == "role admin") {
    if (confirm("Вы точно уверены, что хотите разжаловать администратора?")) $.post("/changeRole", {}).done(function () {
      setTimeout(function () {
        location.reload();
      }, 500);
    });
  } else $.get("/user/", {}).done(function (data) {
